...
class Budget(db.Model):
    ...
    workspace = db.relationship("Workspace", backref=db.backref("budgets", lazy="dynamic"))
    category = db.relationship("Category", backref=db.backref("budgets", lazy="dynamic"))
    created_by = db.relationship("User", foreign_keys=[created_by_user_id])

    def __repr__(self) -> str:
        return f"<Budget ws={self.workspace_id} cat={self.category_id} {self.period} {self.year}-{self.month} limit={self.limit_amount}>"
...
class SavingsPot(db.Model):
    ...
    name = db.Column(db.String(255), nullable=False)
    kind = db.Column(db.String(20), nullable=False, default="pot")
    ...
    workspace = db.relationship("Workspace", backref=db.backref("savings_pots", lazy="dynamic"))
    created_by = db.relationship("User", foreign_keys=[created_by_user_id])

    def __repr__(self) -> str:
        return f"<SavingsPot ws={self.workspace_id} id={self.id} name={self.name} kind={self.kind}>"


class SavingsPotContribution(db.Model):
    __tablename__ = "savings_pot_contributions"

    id = db.Column(db.Integer, primary_key=True)
    pot_id = db.Column(db.Integer, db.ForeignKey("savings_pots.id"), nullable=False)
    user_id = db.Column(db.Integer, db.ForeignKey("users.id"), nullable=False)
    amount = db.Column(db.Numeric(15, 2), nullable=False)
    contribution_date = db.Column(db.Date, nullable=False)
    created_at = db.Column(db.DateTime, default=datetime.utcnow, nullable=False)

    pot = db.relationship("SavingsPot", backref=db.backref("contributions", lazy="dynamic", cascade="all, delete-orphan"))
    user = db.relationship("User", foreign_keys=[user_id])

    def __repr__(self) -> str:
        return f"<SavingsPotContribution pot={self.pot_id} user={self.user_id} amount={self.amount}>"
